/* ************************************************************************* *\
*                Programmierung 1 HS 2015 - Serie 3-1 
...
public class GuessOrc {
	...
	public void start()
	{
		...
		guessInt = getColumnAsInt(guess.charAt(0));
		boolean won = false;
		while(!won) {
			if(movesMade() >= MAX_ATTEMPTS) {
				System.out.println("The Orcs entered your castle! You lost!");
				won = true;
			} else {
				System.out.println("Next guess: ");
				guess = scn.nextLine();
				while(guess.length() > 1 || getColumnAsInt(guess.charAt(0)) == PARSING_FAILED) {
					System.out.println("Your guess was invalid. Please try once more: ");
					guess = scn.nextLine();
				}
				guess.toLowerCase();
				guessInt = getColumnAsInt(guess.charAt(0));
				if(guessInt == mineShaftId) {
					printMap(guessInt, NO_HINT);
					System.out.println("You won!");
					won = true;
				} else {
					printMap(guessInt, calculateHint(guessInt, generator));
				}
			}
		}
	...
	public int calculateHint( int guessedMineShaftId, Random generator){
		...
		if(guessedMineShaftId == mineShaftId) {
			if(/* Orc under stone */ (mineShaftId + movesMade()) % 2 == 1) {
				while(HintId != 0 && HintId != 4 && HintId != 3)/* 0,3,4 */{
				HintId = generator.nextInt(5);
				}
				return HintId;
			}
			if(/* Orc under dirt */ (mineShaftId + movesMade()) % 2 == 0) {
				while(HintId != 0 && HintId != 4 && HintId != 2)/* 0,2,4  */{
				HintId = generator.nextInt(5);
				}
				return HintId;
			}
		...
		if(guessedMineShaftId > mineShaftId) {
			if(/* Orcs > 5 fields away */ guessedMineShaftId - 5 > mineShaftId) {
				if(/* Orc under stone */ (mineShaftId + movesMade()) % 2 == 1) {
					while(HintId != 0 && HintId != 4 && HintId != 3)/* 0,3,4 */{
					HintId = generator.nextInt(5);
					}
			...
			if(/* Orc under Stone */ (mineShaftId + movesMade()) % 2 == 1) {
					while(HintId != 0 && HintId != 3)/* 0,3 */{
					HintId = generator.nextInt(5);
					}
					return HintId;
			}
			if(/* Orc under dirt */ (mineShaftId + movesMade()) % 2 == 0) {
					while(HintId != 0 && HintId != 2)/* 0,2  */{
					HintId = generator.nextInt(5);
					}
					return HintId;
				}
			...
			if(/* Orcs > 5 fields away */ guessedMineShaftId + 5 < mineShaftId) {
				if(/* Orc under stone */ (mineShaftId + movesMade()) % 2 == 1) {
					while(HintId != 1 && HintId != 4 && HintId != 3)/* 1,3,4 */{
					HintId = generator.nextInt(5);
					}
					return HintId;
				}
				if(/* Orc under dirt */ (mineShaftId + movesMade()) % 2 == 0) {
					while(HintId != 1 && HintId != 4 && HintId != 2)/* 1,2,4  */{
					HintId = generator.nextInt(5);
					}
					return HintId;
				}
			...
			if(/* Orc under Stone */ (mineShaftId + movesMade()) % 2 == 1) {
					while(HintId != 1 && HintId != 3)/* 1,3 */{
					HintId = generator.nextInt(5);
					}
					return HintId;
			}
			if(/* Orc under dirt */ (mineShaftId + movesMade()) % 2 == 0) {
					while(HintId != 1 && HintId != 2)/* 1,2  */{
					HintId = generator.nextInt(5);
					}
					return HintId;
				}
...
//		System.out.println("CurrentMove: " + currentMove);
		...
		if( column == mineShaftId ){ //Did we get the orcs?
			...
			System.out.println();
		} else { //If not, check if they have moved at all.
//			if (movesMade() == 0) {
				System.out.println();
				System.out.println();
